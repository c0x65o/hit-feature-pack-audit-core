...
export async function checkAuditCoreAction(request, actionKey) {
    ...
    if (!token) {
        console.log(`[Audit-Core Action Check] ${actionKey}: No token found`);
    ...
    console.log(`[Audit-Core Action Check] ${actionKey}: Checking via ${url}`);
    const res = await fetch(url, {
        ...
        console.log(`[Audit-Core Action Check] ${actionKey}: Fetch error:`, e);
        return null;
    });
    if (!res) {
        console.log(`[Audit-Core Action Check] ${actionKey}: Auth unreachable (no response)`);
        return { ok: false, source: 'auth_unreachable' };
    }
    if (!res.ok) {
        const errorText = await res.text().catch(() => '');
        console.log(`[Audit-Core Action Check] ${actionKey}: Auth returned status ${res.status}:`, errorText);
        return { ok: false, source: `auth_status_${res.status}` };
    }
    const json = (await res.json().catch(() => null));
    const ok = Boolean(json?.has_permission ?? json?.hasPermission ?? false);
    console.log(`[Audit-Core Action Check] ${actionKey}: Result`, { ok, source: json?.source, response: json });
    return { ok, source: String(json?.source || '') || undefined };
}
export async function requireAuditCoreAction(request, actionKey) {
    const result = await checkAuditCoreAction(request, actionKey);
    if (result.ok)
        return null;
