...
export async function GET(request) {
    ...
    if (mode === 'none') {
        ...
        if (user.sub) {
            whereParts.push(eq(auditEvents.actorId, user.sub));
        }
        else {
        ...
        const ownCondition = user.sub ? eq(auditEvents.actorId, user.sub) : sql `false`;
        ...
        if (scopeIds.divisionIds.length) {
          ...
          where ${userOrgAssignments.userKey} = ${auditEvents.actorId}
    ...
    if (actorType)
        whereParts.push(eq(auditEvents.actorType, actorType));
    if (correlationId)
    ...
    if (eventType)
        whereParts.push(eq(auditEvents.eventType, eventType));
    if (outcome)
        whereParts.push(eq(auditEvents.outcome, outcome));
    if (targetKind)
        whereParts.push(eq(auditEvents.targetKind, targetKind));
    if (targetId)
        whereParts.push(eq(auditEvents.targetId, targetId));
    if (sessionId)
        whereParts.push(eq(auditEvents.sessionId, sessionId));
    if (q)
